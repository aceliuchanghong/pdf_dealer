...
def process_pdf(input_pdf_path, output_path, *, rotate_pic=False):
    ...
    latex_result = {}
    start_time = time.time()  # 记录开始时间
    for image in tqdm(images_path, desc="表格识别中......"):
        latex = get_latex_table(image, ip=os.getenv('GOT_OCR_ip'), ocr_type="format")
        latex_result["images/" + image] = latex
    ...
    logger.debug(f"result_md_path: {new_md_path}")
...
if __name__ == '__main__':
    """
    python pdf2md.py --input_pdf_path xx.pdf --output_path /mnt/data/llch/ForMinerU/output
    --rotate_pic
    """
